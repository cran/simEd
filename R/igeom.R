###############################################################################
# R function to visualize the geometric idf evaluated at a provided uniform(0,1)
...
# variates generated, with theoretical geometric superimposed as spike/dots.
###############################################################################
                   ...
                   resetRowsMargins = TRUE)
{
  ...
  fromX <- qgeom(minPlotQuantile, prob)
  toX   <- qgeom(maxPlotQuantile, prob)
  ...
  rows <- sum(showCDF, showPMF, showECDF)
  par(mfrow = c(rows, 1))
  ...
  titleStr <- paste("geometric(prob = ", as.character(prob), ")\n", sep = "")
  if (!is.null(xVals))
    titleStr <- paste(titleStr, "random variate generation", sep = "")

  # for plotting, if quantiles are not 0 and/or 0.99, we need to plot just
  # slightly to the left/right to make the discrete stairstep cdf look right...
  xAxisMin <- fromX
  xAxisMax <- toX
  if (fromX > 0)
    xAxisMin <- fromX - (0.02 * (toX - fromX))
  xAxisMax <- toX   + (0.02 * (toX - fromX))  # since geom is not bound on right
  ...
  exactPMF <- dgeom(fromX:toX, prob)
  exactCDF <- c(pgeom(0:toX, prob), 1)  # add the 1 since geom is unbounded
  ...
  if (showCDF == TRUE)
  {
    ...
    # draw outgoing stub of horizontal and intersecting dotted vertical 
    ...
    xValsPlotted <- rep(0, maxXVal + 1)    # size + 1 distinct x values
    for (i in 1:length(u))
    {
  ...
  if (showECDF == TRUE)
  {
    ...
    else
    {
      ...
      segments(lastKnot, 1, xAxisMax, 1, lwd = lwd, col = color)
