###############################################################################
# R function to visualize the exponential idf evaluated at a provided 
                   ...
                   resetRowsMargins = TRUE)
{
  ...
  fromX <- qexp(minPlotQuantile, rate = rate)
  toX   <- qexp(maxPlotQuantile, rate = rate)
  ...
  titleStr <- paste("exponential(rate = ", as.character(rate), ")\n", sep = "")
  if (!is.null(xVals)) 
  ...
  #    > set.seed(123456); vals = iexp(runif(20),0.01,scale=0.03,showHist=T)
  ...
  if (!is.null(xVals)) 
  {
    ...
    if (is.null(histInfo)) { 
      histInfo <- tryCatch( hist(xVals, breaks = "sturges", plot = FALSE),
                    error = function(err) { 
                      print(err) 
                      stop('iexp: Error internally using hist()') } )
    }
    if (length(histInfo$breaks) > maxBins)
       stop(paste('iexp: Error using hist() -- more than', maxBins, 'bins'))

    options(warn = 1)  # reset to our iexp state: immediate warnings
  ...
  exactCDF <- pexp(x, rate = rate)

  # hang on to the exact pdf, for use in max pdf plot height below and in
  # plotting the pdf...
  exactPDF <- dexp(seq(fromX, toX, by = (toX-fromX)/100), rate = rate)

  # the majority of density may be packed into the first histogram bin...
  ...
  if (showPDF == TRUE)
  {
    ...
    curve(dexp(x, rate = rate), xlim = c(fromX, toX), add = TRUE, lwd = 2)
