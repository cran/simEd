###############################################################################
# R function to visualize the Weibull idf evaluated at a provided uniform(0,1)
...
# generated, with theoretical Weibull superimposed.
###############################################################################
                   ...
                   resetRowsMargins = TRUE)
{
  ...
  fromX <- qweibull(minPlotQuantile, shape = shape, scale = scale)
  toX   <- qweibull(maxPlotQuantile, shape = shape, scale = scale)
  ...
  if (is.null(u)) {
    ...
    xVals <- qweibull(u, shape = shape, scale = scale)
  }
  ...
  titleStr <- paste("Weibull(shape = ", as.character(shape), 
  ...
  #    > set.seed(123456); vals = iweibull(runif(20),0.01,scale=0.03,showHist=T)
  ...
  if (!is.null(xVals)) 
  {
    ...
    if (is.null(histInfo)) { 
      histInfo <- tryCatch( hist(xVals, breaks = "sturges", plot = FALSE),
                    error = function(err) { 
                      print(err) 
                      stop('iweibull: Error internally using hist()') } )
    }
    if (length(histInfo$breaks) > maxBins)
       stop(paste('iweibull: Error using hist() -- more than', maxBins, 'bins'))

    options(warn = 1)  # reset to our iweibull state: immediate warnings
  ...
  # shape = 0.1 and scale = 0.003) using curve() to plot creates a
  ...
  exactCDF <- pweibull(x, shape = shape, scale = scale) 

  # hang on to the exact pdf, for use in max pdf plot height below and in
  # plotting the pdf...
  # when shape parameter is < 1, Weibull is Inf at fromX = 0 -- remove Inf
  exactPDF <- dweibull(seq(fromX, toX, by = (toX-fromX)/100), 
                shape = shape, scale = scale)
  if (shape < 1) { exactPDF <- exactPDF[-1] }

  # for 0 < shape < 1, and, for example, small scale (e.g., scale=0.002),
  # the majority of density is packed into the first histogram bin...
