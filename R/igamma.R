###############################################################################
# R function to visualize the gamma idf evaluated at a provided uniform(0,1)
                   ...
                   respectLayout   = TRUE)
{
  ...
  fromX <- qgamma(minPlotQuantile, shape = shape, scale = scale)
  toX   <- qgamma(maxPlotQuantile, shape = shape, scale = scale)
  ...
  if (is.null(u)) {
    ...
    xVals <- qgamma(u, shape = shape, scale = scale)
  }
  ...
  dispScale <- round(scale, digits = 3)
  if (dispScale == 0) { dispScale <- format(scale, digits = 3, scientific = TRUE) }
  titleStr <- paste("gamma(shape = ", as.character(shape), 
              ", scale = ", as.character(dispScale), ")\n", sep = "")
  if (!is.null(xVals)) 
  ...
  if (!is.null(xVals)) 
  {
    ...
    if (is.null(histInfo)) { 
      histInfo <- tryCatch( hist(xVals, breaks = "sturges", plot = FALSE),
                    error = function(err) { 
                      print(err) 
                      stop('igamma: Error internally using hist()') } )
    }
    if (length(histInfo$breaks) > maxBins)
       stop(paste('igamma: Error using hist() -- more than', maxBins, 'bins'))

    options(warn = 1)  # reset to our igamma state: immediate warnings
  ...
  exactCDF <- pgamma(x, shape = shape, scale = scale) 

  # hang on to the exact pdf, for use in max pdf plot height below and in
  # plotting the pdf...
  # when shape parameter is < 1, gamma is Inf at fromX = 0 -- remove Inf
  exactPDF <- dgamma(seq(fromX, toX, by = (toX-fromX)/100), 
