###############################################################################
# R function to visualize the uniform idf evaluated at a provided uniform(0,1)
...
# generated, with theoretical uniform superimposed.
###############################################################################
                  ...
                  respectLayout   = TRUE)
{
  ...
  fromX <- qunif(minPlotQuantile, min = min, max = max)
  toX   <- qunif(maxPlotQuantile, min = min, max = max)
  ...
  titleStr <- paste("uniform(min = ", as.character(min), 
  ...
  if (!is.null(xVals)) 
  {
    ...
    if (is.null(histInfo)) { 
      histInfo <- tryCatch( hist(xVals, breaks = "sturges", plot = FALSE),
                    error = function(err) { 
                      print(err) 
                      stop('iunif: Error internally using hist()') } )
    }
    if (length(histInfo$breaks) > maxBins)
       stop(paste('iunif: Error using hist() -- more than', maxBins, 'bins'))

    options(warn = 1)  # reset to our iunif state: immediate warnings
  ...
  exactCDF <- punif(x, min = min, max = max)

  # hang on to the exact pdf, for use in max pdf plot height below and in
  # plotting the pdf...
  exactPDF <- dunif(seq(fromX, toX, by = (toX-fromX)/100), 
                min = min, max = max)
